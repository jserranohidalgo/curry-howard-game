...
- **D21 — Keep the engine minimal (2026-08-15).** The first cut of Phase 2 carried three type classes — `Term`, `Rule` and `Calculus` — inherited from the reference implementation. All three are gone. `Term` was strictly redundant: the NJ algebra is at exactly the right granularity, since every destructor acts on a *variable in scope*, so `Ax`, backward `∧.E₁` and the rest are fixed compositions of `Term` operations rather than needing the finer vocabulary — and `subst`, which the sequent-calculus algebras need, was never called. `Rule` was a `PartialFunction` wearing a type class once its algebra moved out. `Calculus` bundled a coalgebra, an algebra and a `Traverse` for a single instance. What remains: the enum, its coalgebra, a `Traverse`, and one file per interpretation. `Partial`, `GameTree` and the search plumbing are **specialised to NJ** rather than generic over the calculus — there is one game calculus, LJT is only ever consulted and never played, and the generality was costing real friction. `Mu` and the search space stay generic, so the oracle can still reuse them. The rule of thumb: reintroduce an abstraction when a second instance actually needs it.
