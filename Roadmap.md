...
- **D21 — Keep the engine minimal (2026-08-15).** The first cut of Phase 2 carried four type classes — `Term`, `Rule`, `Calculus` and `Form` — inherited from the reference implementation. All four are gone. `Term` was strictly redundant: the NJ algebra is at exactly the right granularity, since every destructor acts on a *variable in scope*, so `Ax`, backward `∧.E₁` and the rest are fixed compositions of `Term` operations rather than needing the finer vocabulary — and `subst`, which the sequent-calculus algebras need, was never called. `Rule` was a `PartialFunction` wearing a type class once its algebra moved out. `Calculus` bundled a coalgebra, an algebra and a `Traverse` for a single instance. `Form` was the last to go: a tagless-final formula interface buys the ability to have *alternative representations* of a proposition, and the game has one. Dropping it takes every `[F: Form]` context bound with it, and structural equality — which the rules use constantly, to decide whether a resource's type *is* the goal — becomes the case classes' own `==`. Note what did **not** depend on it: the two readings of a formula were never two instances of a class, they are two functions over one ADT, so "one engine, two syntaxes" survives untouched. What remains: the `Formula` enum, the `NJ` enum, a `Traverse`, and one file per interpretation.

  One further step, and one reversal worth recording. The three *forward* destructors are now a single primitive **`let`** — the cut rule, `val x: A = value; body`, with holes for both children. It is derivable in principle (`let` is `(λx: A. body) value`) but stays primitive, because expanding it makes a move several rules deep, and a coalgebra `Sequent => LazyList[NJ[Sequent]]` can only offer moves that are one rule deep with holes for children. An attempt to expand it away did exactly that damage — the coalgebra, `Mu` and the lazily-unfolded search space all went with it — and was reverted. **The coalgebra is the architecture.** Nesting an `NJ[T]` inside the rule was the other way to keep a forward use to one move, and was rejected for the same kind of reason: it makes the rule set self-recursive, and here recursion comes from the fixpoint and nowhere else.

  Two consequences of `let` being a genuine cut. A forward extraction now costs **two moves** rather than the one specification §4.9 counts — state what you want in scope, then produce it — and the §4.9 playthrough is twelve moves for the same term. And a `let` is identified by *what it binds* rather than by a resource, since the value is a hole and two resources offering an `Either[Q, R]` offer one move between them; so it has no cell in the §3.2 rules table, and Phase 8 has to decide how it is shown. What keeps cut from making search diverge is that the bound type ranges only over what one elimination step could yield from something in scope.

  `Partial`, `GameTree` and the search plumbing are **specialised to NJ** rather than generic over the calculus — there is one game calculus, LJT is only ever consulted and never played, and the generality was costing real friction. `Mu` and the search space stay generic, so the oracle can still reuse them. The rule of thumb: reintroduce an abstraction when a second instance actually needs it.
- **D22 — Natural deduction ships first; LJ becomes another game (2026-08-15).** The `let` rule exposed a real asymmetry (see the open question under Phase 7): forward reasoning is native to a sequent calculus and derived in natural deduction, LJ's left rules carry the subformula property that makes move generation principled rather than heuristic, and §4.9 in LJ is eleven primitive moves against natural deduction's twelve. None of that changes what ships first. The application is for **introductory courses on logic and functional programming**, and natural deduction with algebraic data types is the salient first example — the one where a hole's type drives the move and `x._1` is the obvious way to close a `… : P`. LJ with Herbelin's λ̄ joins the roster *as another game*, at the same level as classical propositional logic and the rest, where its different term assignment is a feature rather than a distraction. Two consequences worth stating: the `let` awkwardness is accepted for the first release as the price of teaching natural deduction, and the logician view must draw **natural deduction**, which rules out the LJ-shaped figure among the Phase 7 options. It also puts a date on D21's rule of thumb — the engine is specialised to NJ until a second calculus needs it, and now we know one is coming.
- **D23 — Contextual help is in scope, and so are hints (2026-08-16).** The interaction spec reserves a Hint slot and says "build none of it", and D20 noted that turning an LJT proof into a suggested next move is proof translation, the hard direction. Both are superseded. **The objection does not apply**, because a hint needs no translation: ask of each legal move whether every hole it opens is provable, and all-provable means the move keeps the game winnable. Sound, terminating, and it reuses the oracle already built. What decides the scope change is who the users are — students, for whom being stuck is the expected state rather than a failure, with a secondary audience of people playing puzzles for fun. Help is therefore not a nicety bolted on at the end but part of what the application *is*. See Phase 10.
...
**The Play screen keeps one selected hole** (2026-08-16), as the interaction spec draws it: the rules table always refers to the selected hole, and the resources panel shows what is in scope there. The console offers every move at every hole in one flat list instead, which is quicker to play — but that is an artifact of the medium. Selecting a hole costs a typed command in a REPL and one click on a hole chip in a browser, so the console's preference is weak evidence for a screen. Three things the flat list would have cost here: hole labels inside the table's cells, a resources panel repeated per hole, and the loss of a focusing device that shows a first-year student one obligation at a time.
...
The reference decision procedure the exit criterion calls for is **in hand** (Phase 2, 2026-08-15): `ipl/ljt` implements Dyckhoff's rules and `Decide.provable` decides intuitionistic provability with no depth bound, because every LJT rule replaces a formula with its parts and so the backward search cannot loop. It settles excluded middle, Peirce's law, the classical de Morgan direction and — the case the game's own search can never reach — double-negation elimination, all in milliseconds. A property test asserts it terminates on random formulas, and another cross-checks the direction that matters for soundness: anything the game manages to finish, LJT calls provable.
...
3. ~~**An LJ-shaped figure**, where every move is one inference natively.~~ **Ruled out by D22:** the course teaches natural deduction, so the logician view cannot draw a different proof system. LJ gets its own game instead.
...
## Phase 10 — Contextual help

Not a phase that runs last: its content accumulates from Phase 4 onwards, and it began in the console client (2026-08-16) where the interaction is cheapest to try. Three escalating levels, available at **every** prompt:

- **`?` — what am I looking at, and what do I do now.** Reads the position: how many holes, what each needs, what is in scope, and what the panels mean. Says something different when a branch is dead.
- **`??` — what these particular moves mean**, in both vocabularies at once (`∨.E` — *program:* pattern match; *logic:* prove your goal twice, once per case), grouped by lesson rather than by rule name so the two disjunction introductions are one entry. Also explains the table's `·` and `—`, since the difference between "does not apply here" and "does not exist" is itself a lesson.
- **`???` — a hint**: which moves keep the goal winnable, which lead nowhere, and — when the position warrants it — *why*. At §4.9's choice point it produces the specification's own lesson without being told it: *"You are being offered Left and Right, and neither works yet. Inspect the disjunction before choosing a disjunct."*

Two honesty constraints the console version already imposes. A `let` can never lose the game, since it adds to scope without touching the goal, so listing it as "keeps the goal winnable" is true but weak — the hint separates *progress* from *harmlessness* rather than mixing them in one list. And when a position cannot be finished, the hint distinguishes a wrong turn (the goal was provable at the start; backtrack) from a goal that was never a theorem, where getting stuck everywhere *is* the answer (§4.7).

*Depends on:* the LJT oracle (Phase 2) for anything at the `???` level. *Done when:* the three levels exist in the web UI as well, in both languages, and the Help screen's six steps (§4 of the handoff) are written against what the levels actually say.
...
- **More systems.** The Home screen already lists the four that follow: classical propositional logic (λμ-calculus, `call/cc`), first-order intuitionistic (λΠ), first-order classical, and multiplicative linear logic. **LJ with Herbelin's λ̄ joins them** (D22) — the same propositional logic under a different proof system, which under Curry–Howard is a different language, and the one where forward reasoning is primitive and proof search is well-behaved. Each is a rule table plus its rendering in both viewpoints — which is the test of whether the engine was really parameterised by the calculus, and LJ is the sharpest such test because its moves decompose where natural deduction's accumulate.
