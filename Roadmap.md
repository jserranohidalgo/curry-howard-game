...
- **A natural-deduction `Calculus[NJ]`** — the game's own rule set, since LJ and LJT are sequent calculi and the game plays natural deduction (specification §2.4 is explicit that these are different languages). Each rule is a `coalg`/`alg` pair: `coalg` says when the rule applies to a sequent and which sub-goals it opens — that is legal-move generation, with partiality doing the applicability test — and `alg` says how the term is assembled from its sub-terms. Both backward and forward destructor use, per §3.2.
...
Two cautions carried over from the analysis. The reused code is **CC BY-SA 4.0** against this project's MIT, so it needs relicensing by its author before it lands here — a formality, but one to perform rather than assume. And its allocation behaviour in a browser is unmeasured: fine for one hole's moves, unknown for the Phase 6 search.
...
- **The reused engine code needs relicensing** (D20). [hablapps/tdd](https://github.com/hablapps/tdd/tree/lambdadays25) is CC BY-SA 4.0 and this project is MIT; share-alike does not flow into a permissive licence. Its author is this project's author, so it is a formality — but it has to be done deliberately, and before Phase 9 makes the repository public.
